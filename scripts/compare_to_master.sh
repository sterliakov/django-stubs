...
# Try to compare with master (fails for brand-new repo without any commits)
ref_hash=$(git rev-parse HEAD) || (write_cache && exit 3)
if [ "$ref_hash" = "$cur_hash" ]; then
    # Already on master; compare to previous commit
    # Fails for repo with single commit
    ref_hash=$(git rev-parse HEAD~1) || (write_cache && exit 3)
fi
